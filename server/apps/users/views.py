...
from ai_assistant.settings import DEFAULT_FROM_EMAIL
...
class RegisterView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            verification_link = f"http://localhost:8000/api/user/activate/{uid}/{token}/"

            # Send email
            subject = "Xác nhận đăng ký tài khoản - Echo Mind"
            html_message = render_to_string("users/confirm_email.html", {"user": user, "activation_link": verification_link})
...
class RequestPasswordResetView(APIView):
    def post(self, request):
        ...
        # Send email
        subject = "Đặt lại mật khẩu cho tài khoản - Echo Mind"
